...
func (_q *SysRoleQuery) QueryMenus() *SysMenuQuery {
...
func (_q *SysRoleQuery) WithMenus(opts ...func(*SysMenuQuery)) *SysRoleQuery {
	query := (&SysMenuClient{config: _q.config}).Query()
...
func (_q *SysRoleQuery) loadMenus(ctx context.Context, query *SysMenuQuery, nodes []*SysRole, init func(*SysRole), assign func(*SysRole, *SysMenu)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(sysmenu.FieldID), joinT.C(sysrole.MenusPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(sysrole.MenusPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(sysrole.MenusPrimaryKey[0]))
...
func (_q *SysRoleQuery) loadUsers(ctx context.Context, query *SysUserQuery, nodes []*SysRole, init func(*SysRole), assign func(*SysRole, *SysUser)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(sysrole.UsersTable)
		s.Join(joinT).On(s.C(sysuser.FieldID), joinT.C(sysrole.UsersPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(sysrole.UsersPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(sysrole.UsersPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
