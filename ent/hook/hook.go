...
// The SysTenantFunc type is an adapter to allow the use of ordinary
// function as SysTenant mutator.
type SysTenantFunc func(context.Context, *ent.SysTenantMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f SysTenantFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.SysTenantMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.SysTenantMutation", m)
}
